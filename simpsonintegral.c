...
int main(){
	...
	double integral=0;
	h=(x2-x1)/n;
	integral=(Fonksiyon_deger_bul(x1,fonksiyon,derece)+Fonksiyon_deger_bul(x2,fonksiyon,derece));
	for(i=1;i<n;i++){
		if((i%2)==1)
		integral=integral+4*(Fonksiyon_deger_bul(x1+(i*h),fonksiyon,derece));
		else if((i%2)==0)
		integral=integral+2*(Fonksiyon_deger_bul(x1+(i*h),fonksiyon,derece));	
	}
